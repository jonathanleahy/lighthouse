...
func (h *StatsHandler) handleOverallStats(w http.ResponseWriter, r *http.Request) {
	...
	response := api.OverallStats{
		ActiveJobs:     int(stats.ActiveJobs), // Convert int64 to int
		QueuedJobs:     int(stats.QueuedJobs), // Convert int64 to int
		CompletedJobs:  stats.CompletedJobs,
		FailedJobs:     stats.FailedJobs,
		ActiveChannels: stats.ActiveChannels,
		Uptime:         stats.Uptime, // Already a string, no need for .String()
